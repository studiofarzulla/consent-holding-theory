...
%
% PAPER: The Doctrine of Consensual Sovereignty
% VERSION: 1.0.0
% DATE: November 2025
% ============================================================================
...
\usepackage[colorlinks=true,
            ...
            breaklinks=true,
            pdftitle={The Doctrine of Consensual Sovereignty: Quantifying Legitimacy in Adversarial Environments},
...
% ============================================================================
% VERSION AND DOI METADATA
% ============================================================================

% Version number and date
\newcommand{\paperver}{1.0.0}
\newcommand{\paperdate}{November 2025}
\newcommand{\paperdoi}{10.5281/zenodo.17626763}
...
\newcommand{\shorttitle}{Consensual Sovereignty}

\pagestyle{fancy}
\fancyhf{} % Clear default

% Running headers (all pages including first)
\fancyhead[L]{\small\href{https://farzulla.org}{farzulla.org}}                           % Left: farzulla.org (clickable)
\fancyhead[R]{\small\href{https://doi.org/\paperdoi}{\paperdoi}}                        % Right: DOI (clickable)

% Footers (all pages including first)
\fancyfoot[L]{\small Murad Farzulla}                                                     % Left: Author name
\fancyfoot[C]{\small\thepage}                                                             % Center: Page number
\fancyfoot[R]{\small v\paperver\ | \paperdate}                                           % Right: Version and date

\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\footrulewidth}{0.4pt}

% First page uses same style (headers now included)
\fancypagestyle{firstpage}{
  \fancyhf{}
  \fancyhead[L]{\small\href{https://farzulla.org}{farzulla.org}}
  \fancyhead[R]{\small\href{https://doi.org/\paperdoi}{\paperdoi}}
...
% Apply first page style (now with headers)
...
\begin{center}
{\Large\bfseries The Doctrine of Consensual Sovereignty: Quantifying Legitimacy in Adversarial Environments}\\[0.5em]
{\large\itshape The Axiom of Consent}\\[1em]
...
% Metadata box
\metadatabox{10.5281/zenodo.17626763}

\vspace{0.5em}

\noindent\textit{Interactive Visualizations}: \url{https://farzulla.org/research/consent-theory/dashboard.html}

\noindent\textit{Methodologies}: Research methodologies and reproducibility practices are documented at \url{https://farzulla.org/methodologies}
...
This is version 1.0.0 (November 2025), the initial preprint release establishing the theoretical foundations of consent-holding theory. Future versions will expand empirical validation with fully quantified historical cases (women's suffrage 1890-1920 planned for v2.0.0).
...
We demonstrate proof-of-concept through computational validation via agent-based simulation and qualitative validation across seven historical domains (suffrage, abolition, labor rights, civil rights, LGBT inclusion, platform governance, climate policy). The framework's measurement challenges—particularly stakes quantification and effective voice operationalization—are acknowledged explicitly in Section~\ref{sec:operationalization} as requiring domain-specific protocols.
...
\clearpage
\twocolumn
\setstretch{1.5}  % Reapply line spacing after column switch
...
The framework proceeds from seven minimal axioms about collective decision-making to five theorems establishing structural necessities (Section~\ref{sec:framework}). Theorem 1 demonstrates consent-holding necessity: wherever decisions occur, some mapping $H_t(d)$ must exist. Theorem 2 establishes inevitable friction: plural preferences guarantee that someone's interests will be compromised unless perfect alignment obtains. Theorem 3 defines legitimacy as stakes-weighted consent alignment, providing an operational metric. Theorem 4 formalizes the competence-consent trade-off, showing legitimacy as a weighted combination $L = w_1 \cdot \alpha + w_2 \cdot P$. Theorem 5 derives a minimal absolutism from value relativism: even if content-level values are frame-dependent, the existence of consent-holding structures remains invariant.
...
\subsection{Definition 3: Legitimacy as Consent Alignment}
...
\subsection{Postulate 4: Competence-Consent Trade-Off}
...
\textbf{Remark on Weight Determination}: The weights $w_1, w_2$ are not free parameters requiring external normative specification, but endogenous scope conditions revealed through constitutional-level decisions. Societies whose weight configurations produce excessive friction face structural pressure to reform. We can characterize admissible weight functions axiomatically: any stable society must satisfy $w_2/w_1 > f(\text{Var}[s_i(d)])$ where $f$ is a function of stakes heterogeneity derived from friction minimization. Future empirical work will estimate weights via:
\begin{equation}
...
\subsection{Theorem 5: Minimal Absolutism from Relativism}
...
where $y(t) = d(t) + \epsilon$ with $\epsilon \sim \mathcal{N}(0, 0.1)$ represents noisy outcome observation, prior precision $\tau_0 = 1.0$ reflects initial belief strength, and observation precision $\tau_{obs,i} = 10 \times s_i^*$ implements stakes-weighted attention.
...
\textbf{Friction Reduction}: All mechanisms dramatically reduce friction under learning dynamics. Stakes-weighted DoCS friction collapses 98.5\% from initial $F = 105.5$ to final $F = 1.6$, achieving lowest terminal friction. Equal voice reduces friction 98.4\% ($F = 113.8 \to 1.8$), plutocracy 98.2\% ($F = 115.9 \to 2.1$), expert rule 96.8\% ($F = 117.5 \to 3.7$), and random assignment 94.3\% ($F = 146.9 \to 8.3$). Friction collapse validates the theoretical prediction that preference alignment toward observed outcomes reduces stakes-weighted deviation.
...
Cross-mechanism comparisons validate Theorem 4's legitimacy function $L = w_1 \cdot \alpha + w_2 \cdot P$: optimal mechanism depends on domain-specific weights. Technical domains with objective performance metrics (infrastructure engineering, public health interventions) rationally assign high $w_2$, favoring expert mechanisms despite consent costs. Value-laden domains (immigration policy, cultural regulations, distributive justice) assign high $w_1$, favoring stakes-weighted or equal voice mechanisms where stakeholder alignment outweighs technical optimization. The framework provides tools for domain-appropriate matching rather than universal prescriptions—no single mechanism dominates across all contexts, but stakes-weighting achieves superior consent alignment when heterogeneous stakes are empirically measured.
...
\caption{Friction reduction under Bayesian learning dynamics. All mechanisms exhibit dramatic friction collapse as agents update preferences toward observed outcomes. Stakes-weighted DoCS (blue) achieves lowest terminal friction ($F = 1.6$, 98.5\% reduction from initial $F = 105.5$). Equal voice (orange) reduces friction 98.4\% ($113.8 \to 1.8$), plutocracy (red) 98.2\% ($115.9 \to 2.1$). Solid lines show mean across 1000 runs; shaded regions indicate 95\% confidence intervals. Friction collapse validates theoretical prediction that preference alignment toward policy outcomes reduces stakes-weighted deviation.}
...
Stakes mode produces highest terminal $\alpha$ (0.893) but lowest friction reduction (72\% versus 98-99\% for learning/social modes), reflecting winner-take-all dynamics: agents whose preferences align with decisions gain stakes, creating self-reinforcing alignment through power concentration rather than preference convergence. This path-dependent outcome raises entrenchment concerns requiring institutional safeguards (term limits, redistribution, mandatory rotation).
...
\textbf{First, convergence speed differs}: Plutocracy requires 22 periods to reach 90\% final $\alpha$ versus DoCS's 18 periods, imposing 4 additional periods of transition costs. During this learning lag, friction remains 2-3$\times$ higher ($F = 4\text{-}8$ versus $1.5\text{-}2$), generating observable instability.
...
\textbf{Reply}: Theorem 5 addresses this. Content-level value relativism (different cultures prefer different $w_1/w_2$ weights) doesn't undermine structural analysis. The framework doesn't prescribe universal weights—it provides measurement tools applicable regardless of normative commitments. Cross-cultural variation in legitimacy functions becomes empirically testable rather than philosophically irresolvable.
...
The author acknowledges the intellectual contributions of scholars in constitutional political economy, social choice theory, and legitimacy studies whose foundational work made this synthesis possible. Particular thanks to the open-source research community for making preprints, working papers, and replication materials freely accessible.

This research benefited from Perplexity AI for efficient literature discovery and source verification, and Anthropic's Claude for assistance with analytical framework development, literature synthesis, computational validation design, and technical writing. All conceptual innovations, theoretical claims, and interpretive judgments remain the author's sole responsibility.

All computational analysis was conducted at Resurrexi Lab, a distributed computing cluster built from consumer-grade hardware (8 nodes, 66 cores, 229GB RAM, 48GB VRAM), demonstrating that rigorous political economy research is accessible without institutional supercomputing infrastructure.
