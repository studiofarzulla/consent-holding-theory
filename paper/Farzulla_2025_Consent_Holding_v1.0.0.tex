...
We demonstrate proof-of-concept through computational validation via agent-based simulation and qualitative validation across seven historical domains (suffrage, abolition, labor rights, civil rights, LGBT inclusion, platform governance, climate policy). \textbf{Methodological note}: The computational models assume agents learn from outcomes (Bayesian updating), which by construction reduces friction as preferences converge—this is not testing whether friction \textit{can} reduce (that follows definitionally) but comparing which consent mechanisms produce superior alignment given plausible behavioral assumptions. The framework's measurement challenges—particularly stakes quantification and effective voice operationalization—are acknowledged explicitly in Section~\ref{sec:operationalization} as requiring domain-specific protocols.
...
Cross-mechanism comparisons validate Postulate 4's legitimacy function $L = w_1 \cdot \alpha + w_2 \cdot P$: optimal mechanism depends on domain-specific weights. Technical domains with objective performance metrics (infrastructure engineering, public health interventions) rationally assign high $w_2$, favoring expert mechanisms despite consent costs. Value-laden domains (immigration policy, cultural regulations, distributive justice) assign high $w_1$, favoring stakes-weighted or equal voice mechanisms where stakeholder alignment outweighs technical optimization. The framework provides tools for domain-appropriate matching rather than universal prescriptions—no single mechanism dominates across all contexts, but stakes-weighting achieves superior consent alignment when heterogeneous stakes are empirically measured.
